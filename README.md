...
| 그래프 노드 | **250,416** | 그래프 엣지 | **1,125,248** |
...
> **재현 시 주의 — 선택적 의존성이 없으면 폴백이 다른 수치를 낸다.**
> - `networkx>=3.0`(`pip install -e system[graph]`) 없이 그래프를 빌드하면 동일 (출발,도착) 쌍이
>   병합되지 않아 엣지가 1,167,276개로 집계된다. 문서의 1,125,248 은 networkx 백엔드 기준이다.
> - `shapely>=2.0`(`pip install -e system[geo]`) 없이 인접을 재계산하면 정점 일치 폴백이 쓰여
>   인접 쌍 수가 달라진다. 현재 1,310 쌍은 shapely 경계 교차 기준이다.
> - `system/data/meta/graph-stats.json` 의 `backend` 필드로 어느 쪽으로 빌드됐는지 확인할 수 있다.
