...
실데이터로 화면 16종이 동작한다. 데이터 경로 한 줄만 바꾸면 가상(mock) 데이터로도 돌릴 수 있다.
...
- 의존성: 없음. 지도·차트·그래프 라이브러리는 `viz/public/vendor/` 에 내장해 오프라인에서도 동작한다.
- 행정경계: 국토교통부 V-World(`LT_C_ADSIGG_INFO`, 2026-09-01) 기준 기초자치단체 227곳 전수.
...
> 자산별 3열 비교표(DB / 배포본 / 완전판)는 [22 완성도 최종점검 §1.7](../docs/22_완성도_최종점검.md) 에 있다.
...
전체 대조표는 [22 완성도 최종점검](../docs/22_완성도_최종점검.md) 을 본다.
...
자산별 3열 커버리지표(DB / 배포본 / 완전판)는 [22 완성도 최종점검 §1.7](../docs/22_완성도_최종점검.md) 에 있다.
