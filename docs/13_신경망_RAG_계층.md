...
## 부록. 2026-08-23 재점검 — kNN 커버리지와 '같은 지자체' 편향

화면의 '모델 간 일치도' 가 Jaccard 0.000 을 그린다는 보고로 시작해 두 가지를 찾았다.

### 1) 세 모델의 kNN 이 서로 다른 부분집합에서 멈춰 있었다

| | node_embeddings | neural_similarity(kNN) |
|---|---:|---:|
| graphsage-numpy | 154,310 | 154,310 |
| metapath2vec-numpy | 154,310 | **30,000** |
| node2vec-numpy | 154,310 | **3,432** |

세 모델 모두 **같은 154,310개 조례의 임베딩**을 갖고 있는데 kNN 만 달랐다.
세 모델을 모두 가진 조례는 653건(0.33%)뿐이라 일치도를 계산할 표본 자체가 없었다.

원인은 `build_neural_similarity(max_items=6000)` 의 전쌍비교 상한이다. 상한 밖 노드에는
행이 생기지 않는다 — 그 함수 docstring 이 이미 이 함정을 경고하고 있었다.

임베딩은 그대로 두고 kNN 만 `max_items=None` 으로 다시 구웠다(`system/make_neural_full.py`).
**커버리지 653 → 154,310건.**

> 주의 — `EmbeddingResult` 를 labels 없이 만들면 `node_kind` 가 NULL 로 적재된다.
> 값은 정확하지만 `node_kind='Ordinance'` 로 거르는 조회에 안 잡힌다. 실제로 한 번 겪었고
> id 접두어(`ordinance:` / `region:`)로 UPDATE 해 복구했다.

### 2) 진짜 문제는 커버리지가 아니라 '같은 지자체' 편향이었다

커버리지를 채운 뒤에도 모델 간 Jaccard 는 0.005~0.009 에 머물렀다. 원인을 재보니:

| 모델 | top-10 중 같은 지자체 비율 | 10개가 전부 같은 지자체인 조례 |
|---|---:|---:|
| graphsage-numpy | 72.2% | 169/300 |
| node2vec-numpy | 88.7% | 266/300 |
| metapath2vec-numpy | 92.4% | 277/300 |

임베딩이 `HAS_ORDINANCE`(지자체→조례) 엣지에 지배돼, '유사 조례' 를 물으면 대부분
**같은 시·군의 다른 조례**가 돌아왔다. 실제 사례 —
`가평군 기후위기 대응을 위한 탄소중립·녹색성장 조례` 의 이웃이
가평군 기업유치·도로점용료·생활폐기물 조례였다.

반면 분야 신호가 뚜렷한 조례에서는 잘 작동했다.
`강원특별자치도 반려동물 보호 및 학대방지 조례` → graphsage 5/5, node2vec 5/5 가 타 지자체의
반려동물 조례였다. 즉 모델이 망가진 게 아니라 **지역 엣지가 분야 신호를 덮는 조례가 많았던 것**이다.

### 조치 — 후보에서 같은 지자체를 제외한다

이 시스템의 핵심 질문은 "다른 지자체는 이걸 어떻게 만들었나" 다. 같은 지자체 이웃은
그 답이 되지 않는다(그건 지역 상세 화면이 이미 보여준다). 재학습 없이 kNN 후보에서
같은 `region_id` 를 제외하고 다시 골랐다 — `system/make_neural_cross.py`.
비교가 필요하면 `--keep-same-region` 으로 종전 동작을 재현할 수 있다.

낮은 모델 간 일치도 자체는 **결함이 아니다.** 전국에 같은 분야 조례가 수백 건 있으면
그중 어느 10건을 고르든 타당하므로, 서로 다른 10건을 고르는 것이 정상이다.
화면은 이 점을 설명하고, 여러 모델이 함께 꼽은 이웃(consensus)을 따로 표시한다.
